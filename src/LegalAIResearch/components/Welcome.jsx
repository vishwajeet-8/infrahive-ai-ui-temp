...
// import { Search, Download, ExternalLink, FileText, Scale, Gavel, BookOpen, Loader2 } from 'lucide-react';
...
//       if (!data.indakanoon && !data.perplexity) {
...
//   //   setPdfError(null);
//   //   try {
//   //     const indianKanoonPath = new URL(url).pathname;
//   //     const proxyUrl = `/indiankanoon${indianKanoonPath}`;
...
//   //     const docContent = xmlDoc.querySelector('doc')?.textContent;
//   //     if (!docContent) {
//   //       throw new Error('No document content found in response');
//   //     }

//   //     let pdfBytes;
//   //     if (/^[A-Za-z0-9+/=]+$/.test(docContent)) {
...
//   //       }
//   //     } else {
//   //       pdfBytes = new Uint8Array(docContent.length);
//   //       for (let i = 0; i < docContent.length; i++) {
//   //         pdfBytes[i] = docContent.charCodeAt(i) & 0xff;
//   //       }
//   //     }
...
//   //       setPdfData(pdfBytes);
//   //     } else {
//   //       throw new Error('Document content does not appear to be a valid PDF');
//   //     }
//   //   } catch (error) {
...
// //         'Content-Type': 'application/json',
// //       },
// //       body: JSON.stringify({ doc_id: caseId })
// //     });
...
//     console.log('Fetching PDF from:', proxyUrl); // Log URL being fetched
...
//     const responseText = await response.text();
//     console.log('Raw XML response:', responseText); // Log full XML response

//     const parser = new DOMParser();
//     const xmlDoc = parser.parseFromString(responseText, 'text/xml');

//     const docContent = xmlDoc.querySelector('doc')?.textContent;
//     console.log('docContent (first 200 chars):', docContent?.substring(0, 200)); // Log beginning of content

//     if (!docContent) {
//       console.error('No doc content found in XML:', xmlDoc);
//       throw new Error('No document content found in response');
//     }
...
//     // Log PDF header for verification
//     console.log('PDF header bytes:',
...
//   //     } else {
//   //       parts.push(
//   //         <span key={`semi-bold-${match.index}`} style={{ fontWeight: 600 }}>
//   //           {match[3]}
//   //         </span>
//   //       );
//   //     }
...
//     parts.push(
//       <strong key={`bold-tag-${match.index}`}>
//         {match[1]}
//       </strong>
//     );
...
//         styleParts.push(remainingText.substring(styleLastIndex, styleMatch.index));
//       }

//       if (styleMatch[0].startsWith('**')) {
//         styleParts.push(
//           <strong key={`bold-md-${styleMatch.index}`}>
//             {styleMatch[2]}
//           </strong>
//         );
//       } else {
//         styleParts.push(
//           <span key={`semi-bold-md-${styleMatch.index}`} style={{ fontWeight: 600 }}>
//             {styleMatch[3]}
//           </span>
//         );
//       }
...
//                           <span>{processStyledText(line.replace(/^[\*\-]\s*/, ''))}</span>
//                         </div>
//                       );
//                     }
...
//           </div>
//           <h3 className="text-lg font-medium text-gray-900 mb-2">No case law found</h3>
//           <p className="text-gray-600">Please try a different query or modify your search terms</p>
//         </div>
//       );
//     }
...
//           <div key={`case-${index}`} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden transition-all duration-200 hover:shadow-md">
//             <div className="p-6">
...
//                   <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Case ID</p>
...
//                 <div className="bg-gray-50 p-3 rounded-lg">
//                   <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Court</p>
//                   <p className="mt-1 text-sm font-medium text-gray-900">
...
//                   <div className="bg-gray-50 p-3 rounded-lg">
//                     <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Bench</p>
//                     <p className="mt-1 text-sm font-medium text-gray-900">
//                       {caseItem.bench.join(', ')}
//                     </p>
//                   </div>
...
//                 <div className="bg-gray-50 p-3 rounded-lg">
//                   <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Citations</p>
//                   <p className="mt-1 text-sm font-medium text-gray-900">
//                     {caseItem.numcites || '0'}
//                   </p>
//                 </div>
//                 <div className="bg-gray-50 p-3 rounded-lg">
//                   <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Cited by</p>
//                   <p className="mt-1 text-sm font-medium text-gray-900">
...
//               {caseItem.headline && (
//                 <div className="mt-6">
//                   <h4 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Summary</h4>
//                   <div className="prose prose-sm max-w-none text-gray-700 bg-blue-50 rounded-lg p-4">
//                     {caseItem.headline.split('\n').map((line, lineIndex) => (
//                       <p key={`headline-${lineIndex}`} className={lineIndex > 0 ? "mt-2" : ""}>
//                         {processStyledText(line)}
//                       </p>
//                     ))}
//                   </div>
...
//                       </div>
//                       <div className="ml-3">
//                         <h4 className="text-sm font-medium text-blue-800">Case Document</h4>
//                         <p className="text-xs text-blue-600">View full case details</p>
//                       </div>
//                     </div>
//                     <div className="mt-4">
//                       <a
...
//                       </div>
//                       <div className="ml-3">
//                         <h4 className="text-sm font-medium text-gray-800">Court Copy</h4>
//                         <p className="text-xs text-gray-600">Original court document</p>
//                       </div>
//                     </div>
//                     <div className="mt-4">
//                       {!pdfData || currentCaseId !== caseItem.tid ? (
//                         <button
//                           onClick={() => loadPdfData(caseItem.cour_copy, caseItem.tid)}
//                           disabled={loadingPdf && currentCaseId === caseItem.tid}
...
//               Example: Keyword "Supreme Court of India", Query "what are the major judgements on merger and disputes."
//             </div>
//           </form>
//         </div>
...
//                 {searchResults && (
//                   <>
//                     {activeTab === 'indakanoon' && formatIndiaKanoonResults(searchResults.indakanoon)}
//                     {activeTab === 'perplexity' && formatPerplexityResults(searchResults.perplexity)}
//                   </>
//                 )}
//               </div>
...
const KnowledgeData = () => {
  ...
  //     const proxyUrl = `/indiankanoon/origdoc/${caseId}/`;
  ...
  //     console.log('docContent (first 200 chars):', docContent.substring(0, 200)); // Debug log
  ...
  //     const proxyUrl = `http://localhost:3001/api/proxy/indiankanoon/${caseId}`;
  //     console.log('Fetching PDF from proxy URL:', proxyUrl); // Debug log
