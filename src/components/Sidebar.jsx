...
const Sidebar = ({
  ...
  const sessions = Object.entries(legalSessions).map(([id, messages]) => ({
    id,
    title: messages[0]?.text?.slice(0, 30) || "New Session",
    timestamp: messages[0]?.timestamp || "Unknown",
  }));
  ...
  const startNewConversation = () => {
    ...
    navigate(
      `/workspaces/${workspaceId}/legal-document-intelligence/${newSessionId}`
    );
  };
      ...
      <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        >
          ...
          <nav className="flex-1 p-3 overflow-y-auto custom-scrollbar">
            ...
            {/* Recent Conversations with enhanced design */}
            {isExpanded && sessions.length > 0 && (
              <div className="mt-4 pt-3 border-t border-gray-200/50">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Link
                    to={`/workspaces/${workspaceId}/Recent-conversation`}
                ...
                <div className="space-y-1">
                  {sessions
                    ...
                    .map((s) => (
                      <Link
                        key={s.id}
                        to={`/workspaces/${workspaceId}/legal-document-intelligence/${s.id}`}
                        className="block p-2 rounded-xl hover:bg-gray-50/80 transition-all duration-200 group border border-transparent hover:border-gray-200/50 backdrop-blur-sm"
                      >
                        <div className="flex items-start space-x-2">
                          <div className="w-6 h-6 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-200">
                            <MessageSquare className="w-3 h-3 text-white" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-semibold text-gray-800 truncate group-hover:text-gray-900">
                              {s.title}
                            </div>
                            <div className="text-xs text-gray-500 group-hover:text-gray-600">
                              {s.timestamp}
                          ...
                          <Star className="w-3 h-3 text-gray-300 group-hover:text-yellow-400 transition-colors duration-200" />
                        </div>
                      </Link>
                    ))}
                </div>
          ...
          <div className="p-4 border-t border-gray-200/50 flex-shrink-0 space-y-3 bg-gradient-to-r from-white/90 to-gray-50/90 backdrop-blur-sm">
            ...
            {/* {userCredentials?.role === "Owner" && (
              ...
              >
                ...
                <div className="flex-shrink-0 relative z-10 group-hover:rotate-90 transition-transform duration-300">
                  <Plus size={18} />
                </div>
                {isExpanded && (
                  <div className="ml-3 text-left relative z-10">
