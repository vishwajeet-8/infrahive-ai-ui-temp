...
const AIFloatingWindow = ({ isOpen, onClose, onInsert }) => {
  ...
  const templateOptions = [
    {
      id: "1",
      name: "Template 1",
      uri: "https://generativelanguage.googleapis.com/v1beta/files/4z6yi1w09xnm",
    },
    {
      id: "2",
      name: "Template 2",
  ...
  const handleGenerate = async () => {
    ...
    try {
      const files = selectedTemplates
        .map((id) => templateOptions.find((t) => t.id === id)?.uri)
        .filter(Boolean);
      ...
      const response = await fetch("http://20.51.215.177:8000/draft", {
        method: "POST",
        headers: {
      ...
      <div className="bg-white rounded-xl shadow-2xl border border-gray-300 w-[640px] max-h-[550px] overflow-hidden flex flex-col">
        ...
        <div className="grid grid-cols-2 h-[450px]">
                ...
                {templateOptions.map((template) => (
                  ...
                  >
                    <input
                      ...
                      aria-checked={selectedTemplates.includes(template.id)}
          ...
          <div className="p-4 flex flex-col">
            ...
            {/* Content Display */}
            <div className="flex-1 overflow-y-auto h-[calc(100%-2rem)] border border-gray-300 bg-gray-100 rounded-lg p-3">
              {error ? (
